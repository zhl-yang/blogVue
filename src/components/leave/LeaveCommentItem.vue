    ...
    <div>
      ...
      <div class="me-reply-list">
        ...
        <div class="me-view-comment-write" v-show="commentShow">
          ...
          <el-button style="margin-left: 8px" @click="publishLeaveComment()" type="text">留言</el-button>
        </div>
      </div>
    ...
    methods: {
      ...
      publishLeaveComment() {
        ...
        if(!that.$store.state.token){
          return that.$message({type: 'error', message: '请登录后再留言', showClose: true})
        }

        publishLeaveComment(that.reply).then(data => {
